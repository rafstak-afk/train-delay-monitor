...
function timeSubBlock(real,planned,delay,label,state,compact){
  if(!planned)return '';
  ...
  return labelHtml+'<div class="time'+sizeCls+' '+cls+'">'+esc(main)+'</div>'+(show?'<div class="station-meta plan-strike">plan '+esc(planned)+'</div>':'');
}
function renderTime(s,state){
  ...
  if(hasBoth){
    ...
    return '<div class="time-stack">'
      +timeSubBlock(arrivalReal,s.plannedArrival,s.arrivalDelay,'Przyj.',state,true)
      +timeSubBlock(departureReal,s.plannedDeparture,s.departureDelay,'Odj.',state,true)
      +'</div>';
  }
