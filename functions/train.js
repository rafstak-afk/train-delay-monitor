...
async function findCourseFromOpenedContext(train,idp){
  ...
  if(!hit)return null;
  const rawSchedule=hit.scheduleId||hit.scheduledId||hit.scheduleID||hit.routeId||'';
  const schedule=/^20\d{2}$/.test(String(rawSchedule).trim())?'':rawSchedule;
