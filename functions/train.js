...
function renderTime(s,state){
  ...
  if(hasBoth){
    ...
    return '<div class="time-stack">'
      +timeSubBlock(arrivalReal,s.plannedArrival,s.arrivalDelay,'Przyj.',state)
      +'<div class="time-stack-sep"></div>'
      +timeSubBlock(departureReal,s.plannedDeparture,s.departureDelay,'Odj.',state)
      +'</div>';
  }
